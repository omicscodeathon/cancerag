...
class MolecularAnalyzer:
    ...
    def analyze_descriptor_bias_correlations(self, df: pd.DataFrame) -> Dict:
        ...
        for desc in numeric_descriptors:
            if not df[desc].isna().all():
                corr, p_value = stats.pearsonr(
                    df[desc].fillna(df[desc].median()), bias_numeric
                )
                correlations[desc] = {
                    "correlation": corr,
                    "p_value": p_value,
        ...
        sorted_correlations = sorted(
            correlations.items(), key=lambda x: x[1]["abs_correlation"], reverse=True
        )
        ...
        for desc in numeric_descriptors[:20]:  # Limit to first 20 for performance
            if not df[desc].isna().all():
                ...
                shapiro_stat, shapiro_p = stats.shapiro(
                    df[desc].dropna().sample(min(5000, len(df)))
                )
    ...
    def analyze_descriptor_clustering(self, df: pd.DataFrame) -> Dict:
        ...
        results["pca_cumulative_variance"] = np.cumsum(
            pca.explained_variance_ratio_
        ).tolist()
        results["n_components_95"] = (
            np.argmax(np.cumsum(pca.explained_variance_ratio_) >= 0.95) + 1
        )

        # Create visualization
        self._plot_pca_analysis(
            pca_result, pca.explained_variance_ratio_, df["primary_bias_label"]
        )

        return results

    def _plot_basic_properties_distribution(
        self, df: pd.DataFrame, basic_props: List[str]
    ) -> None:
        ...
        for i, prop in enumerate(basic_props):
            if prop in df.columns and i < len(axes):
                ...
                axes[i].set_title(
                    f"{prop.replace('_', ' ').title()} Distribution", fontweight="bold"
                )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "basic_properties_distribution.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()

    def _plot_basic_properties_correlation(
        self, df: pd.DataFrame, basic_props: List[str]
    ) -> None:
        ...
        if len(available_props) > 1:
            ...
            plt.title(
                "Basic Molecular Properties Correlation Matrix",
                fontsize=14,
                fontweight="bold",
            )
            plt.tight_layout()
            plt.savefig(
                self.output_dir / "basic_properties_correlation.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def _plot_descriptor_bias_correlations(self, top_correlations: List[Tuple]) -> None:
        ...
        plt.yticks(
            ...
            [desc.replace("_", " ").title() for desc in descriptors],
        )
        plt.xlabel("Correlation with Bias")
        plt.title(
            "Top 20 Molecular Descriptors Correlated with Bias",
            fontsize=14,
            fontweight="bold",
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "descriptor_bias_correlations.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()

    def _plot_descriptor_distributions(
        self, df: pd.DataFrame, descriptors: List[str]
    ) -> None:
        ...
        for i, desc in enumerate(descriptors):
            if i < len(axes) and desc in df.columns:
                ...
                if not df_clean.empty:
                    sns.boxplot(
                        data=df_clean, x="primary_bias_label", y=desc, ax=axes[i]
                    )
                    axes[i].set_title(
                        f"{desc.replace('_', ' ').title()}", fontweight="bold"
                    )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "descriptor_distributions.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _plot_pca_analysis(
        ...
        ax1.plot(
            range(1, len(explained_variance) + 1), np.cumsum(explained_variance), "bo-"
        )
