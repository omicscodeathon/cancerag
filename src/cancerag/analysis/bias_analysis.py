...
class BiasAnalyzer:
    ...
    def analyze_bias_consistency(self, df: pd.DataFrame) -> Dict:
        ...
        results['consistency_percentage'] = (1 - len(multi_bias_ligands) / len(df)) * 100
    ...
    def _plot_bias_distribution(self, bias_counts: pd.Series, bias_percentages: pd.Series) -> None:
        """Create bias distribution plots."""
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
        
        # Bar plot
        bars = ax1.bar(bias_counts.index, bias_counts.values, color=sns.color_palette("husl", len(bias_counts)))
        ...
        # Pie chart
        ax2.pie(bias_counts.values, labels=bias_counts.index, autopct='%1.1f%%', startangle=90)
        ax2.set_title('Bias Label Distribution (Percentage)', fontsize=14, fontweight='bold')
        
        plt.tight_layout()
        plt.savefig(self.output_dir / 'bias_distribution.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def _plot_receptor_bias_heatmap(self, bias_matrix: pd.DataFrame, level: str) -> None:
        ...
        sns.heatmap(bias_matrix_norm, annot=True, fmt='.1f', cmap='YlOrRd', 
                   cbar_kws={'label': 'Percentage (%)'})
        
        plt.title(f'Bias Distribution by {level.title()}', fontsize=14, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / f'bias_heatmap_{level}.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_bias_pathway_analysis(self, crosstab: pd.DataFrame) -> None:
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'bias_pathway_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
