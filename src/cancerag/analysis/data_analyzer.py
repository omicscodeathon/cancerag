...
class DataAnalyzer:
    ...
    def __init__(self, config: Dict):
        ...
        docking_results_path = (
            Path(self.paths["reports"]) / "docking_results" / "affinity_comparison.csv"
        )
        ...
        self.docking_analyzer = DockingAnalyzer(
            str(self.output_dir), str(docking_results_path)
        )
    ...
    def load_dataset(self) -> pd.DataFrame:
        ...
        descriptor_aliases = {
            "molecular_weight": ["MW", "MolWt"],
            "logp": ["LogP", "MolLogP"],
            "hba": ["HBA"],
            "hbd": ["HBD"],
            "rings": ["Rings", "RingCount"],
            "tpsa": ["TPSA"],
        }

        for canonical, aliases in descriptor_aliases.items():
    ...
    def generate_summary_report(self, df: pd.DataFrame) -> Dict:
        ...
        summary = {
            ...
            "receptor_family_distribution": df["receptor_family"]
            .value_counts()
            .to_dict(),
        ...
        if self.results:
            summary["analysis_results"] = {
                "bias_analysis": {
                    "class_imbalance_ratio": self.results.get("bias_analysis", {})
                    .get("distribution", {})
                    .get("class_imbalance_ratio", "N/A"),
                    "bias_consistency": self.results.get("bias_analysis", {})
                    .get("consistency", {})
                    .get("consistency_percentage", "N/A"),
                },
                "molecular_analysis": {
                    "descriptor_count": len(
                        self.molecular_analyzer.get_molecular_descriptors(df)
                    ),
                    "top_correlations": len(
                        self.results.get("molecular_analysis", {})
                        .get("bias_correlations", {})
                        .get("top_correlations", {})
                    ),
                },
                "docking_analysis": {
                    "receptor_count": len(
                        self.docking_analyzer.get_docking_columns(df)
                    ),
                    "affinity_stats": len(
                        self.results.get("docking_analysis", {})
                        .get("affinity_distributions", {})
                        .get("affinity_stats", {})
                    ),
                },
                "receptor_analysis": {
                ...
                "statistical_analysis": {
                    "correlation_pairs": len(
                        self.results.get("statistical_analysis", {})
                        .get("correlations", {})
                        .get("high_correlation_pairs", [])
                    ),
                    "significant_differences": sum(
                        1
                        for test in self.results.get("statistical_analysis", {})
                        .get("bias_differences", {})
                        .get("statistical_tests", {})
                        .values()
                        if isinstance(test, dict)
                        and test.get("significant_difference", False)
                    ),
                },
            }
    ...
    def run_comprehensive_analysis(self) -> Dict:
        ...
        """
        # Check if analysis already exists (idempotent behavior)
        summary_path = self.output_dir / "analysis_summary.json"
        if summary_path.exists():
            logger.info(
                f"Data analysis already exists at {summary_path}. Loading existing results..."
            )
            try:
                with open(summary_path, "r") as f:
                    existing_results = json.load(f)
                logger.info("Successfully loaded existing analysis results.")
                return existing_results
            except Exception as e:
                logger.warning(
                    f"Could not load existing analysis results: {e}. Re-running analysis..."
                )
        ...
        logger.info(
            f"Comprehensive data analysis completed in {total_time:.2f} seconds"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Total analysis time: {results.get('summary', {}).get('analysis_time', 'N/A')}"
    )
