...
class StatisticalAnalyzer:
    ...
    def analyze_correlations(self, df: pd.DataFrame) -> Dict:
        ...
        if len(valid_cols) > 100:
            ...
            rdkit_cols = [col for col in valid_cols if any(col.startswith(prefix) for prefix in 
                          ['Max', 'Min', 'Mol', 'Num', 'BCUT', 'Chi', 'Kappa', 'PEOE', 'SMR', 'EState'])]
            
            selected_cols = basic_descriptors + rdkit_cols[:50]  # Limit to 50 RDKit features
        ...
        for feature in key_features:
            if feature in numeric_cols:
                ...
                for bias in bias_groups:
                    group_data.append(df[df['primary_bias_label'] == bias][feature].dropna())
                ...
                try:
                    ...
                    statistical_tests[feature] = {
                        ...
                        'group_means': {name: data.mean() for name, data in zip(group_names, group_data)},
                        'group_stds': {name: data.std() for name, data in zip(group_names, group_data)}
                    }
                except Exception as e:
                    logger.warning(f"Could not perform Kruskal-Wallis test for {feature}: {e}")
    ...
    def _plot_distribution_analysis(self, df: pd.DataFrame, features: List[str]) -> None:
        ...
        for i, feature in enumerate(features):
            if i < len(axes) and feature in df.columns:
                ...
                axes[i].axvline(mean_val + std_val, color='orange', linestyle=':', alpha=0.7, label=f'+1σ: {mean_val + std_val:.2f}')
                axes[i].axvline(mean_val - std_val, color='orange', linestyle=':', alpha=0.7, label=f'-1σ: {mean_val - std_val:.2f}')
                axes[i].legend()
    ...
    def _plot_bias_group_differences(self, df: pd.DataFrame, features: List[str]) -> None:
        ...
        for i, feature in enumerate(features):
            if i < len(axes) and feature in df.columns:
                ...
                axes[i].set_title(f'{feature.replace("_", " ").title()} by Bias Group', fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'bias_group_differences.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_missing_data_analysis(self, missing_by_column_pct: pd.Series) -> None:
        ...
        if len(top_missing) > 0:
            bars = plt.bar(range(len(top_missing)), top_missing.values, color='lightcoral')
    ...
    def _plot_feature_importance(self, feature_importance: pd.DataFrame) -> None:
        ...
        bars = plt.barh(range(len(feature_importance)), feature_importance['mutual_info'], 
