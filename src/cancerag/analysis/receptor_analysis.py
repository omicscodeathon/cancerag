...
class ReceptorAnalyzer:
    ...
    def analyze_receptor_coverage(self, df: pd.DataFrame) -> Dict:
        ...
        for receptor in df["receptor"].unique():
            ...
            receptor_quality[receptor] = {
                ...
                "bias_distribution": receptor_data["primary_bias_label"]
                .value_counts()
                .to_dict(),
    ...
    def analyze_receptor_subtypes(self, df: pd.DataFrame) -> Dict:
        ...
        subtype_bias = (
            df.groupby(["receptor_subtype", "primary_bias_label"])
            .size()
            .unstack(fill_value=0)
        )
        ...
        for subtype in df["receptor_subtype"].unique():
            if pd.notna(subtype):
                ...
                subtype_coverage[subtype] = {
                    ...
                    "receptor_family": subtype_data["receptor_family"].iloc[0]
                    if len(subtype_data) > 0
                    else "Unknown",
                    "bias_categories": subtype_data["primary_bias_label"].nunique(),
                    "bias_distribution": subtype_data["primary_bias_label"]
                    .value_counts()
                    .to_dict(),
                }
        ...
        # Create visualizations
        self._plot_receptor_subtype_analysis(
            subtype_counts.head(15), subtype_bias_pct.head(15)
        )
    ...
    def _identify_coverage_gaps(self, df: pd.DataFrame) -> Dict:
        ...
        receptor_counts = df["receptor"].value_counts()
        low_coverage_receptors = receptor_counts[
            receptor_counts < 5
        ]  # Less than 5 ligands
        ...
        family_counts = df["receptor_family"].value_counts()
        low_coverage_families = family_counts[
            family_counts < 10
        ]  # Less than 10 ligands

        gaps["low_coverage_families"] = low_coverage_families.to_dict()

        return gaps

    def _plot_receptor_family_distribution(
        self, family_counts: pd.Series, family_percentages: pd.Series
    ) -> None:
        ...
        bars = ax1.bar(
            ...
            color=sns.color_palette("husl", len(family_counts)),
        )
        ...
        for bar, count, pct in zip(
            bars, family_counts.values, family_percentages.values
        ):
        ...
        ax2.set_title(
            "Receptor Family Distribution (Percentage)", fontsize=14, fontweight="bold"
        )

        plt.tight_layout()
        plt.savefig(
            self.output_dir / "receptor_family_distribution.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _plot_family_bias_heatmap(self, family_bias_pct: pd.DataFrame) -> None:
        ...
        plt.title(
            "Bias Distribution by Receptor Family", fontsize=14, fontweight="bold"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "family_bias_heatmap.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_individual_receptor_distribution(self, top_receptors: pd.Series) -> None:
        ...
        bars = plt.bar(
            range(len(top_receptors)),
            top_receptors.values,
        ...
        plt.xticks(
            range(len(top_receptors)),
            [
                receptor.replace(" receptor", "").title()
                for receptor in top_receptors.index
            ],
            rotation=45,
            ha="right",
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "individual_receptor_distribution.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _plot_receptor_bias_heatmap(self, receptor_bias_pct: pd.DataFrame) -> None:
        ...
        plt.title(
            "Bias Distribution by Receptor (Top 15)", fontsize=14, fontweight="bold"
        )
        plt.xlabel("Bias Label")
        plt.ylabel("Receptor")
        plt.xticks(rotation=45)
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "receptor_bias_heatmap.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_receptor_coverage(self, receptor_quality: Dict) -> None:
        ...
        ligand_counts = [
            receptor_quality[receptor]["ligand_count"] for receptor in receptors
        ]
        bias_categories = [
            receptor_quality[receptor]["bias_categories"] for receptor in receptors
        ]
        ...
        ax2.hist(
            bias_categories,
            bins=range(1, max(bias_categories) + 2),
            alpha=0.7,
            color="lightcoral",
            edgecolor="black",
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "receptor_coverage.png", dpi=300, bbox_inches="tight"
        )
        plt.close()

    def _plot_receptor_subtype_analysis(
        self, subtype_counts: pd.Series, subtype_bias_pct: pd.DataFrame
    ) -> None:
        ...
        # Subtype distribution
        bars = ax1.bar(
            range(len(subtype_counts)),
            subtype_counts.values,
            color=sns.color_palette("husl", len(subtype_counts)),
        )

        ax1.set_title(
            "Top 15 Receptor Subtypes by Ligand Count", fontsize=14, fontweight="bold"
        )
        ...
        ax1.set_xticklabels(
            [
                subtype.replace(" receptor", "").title()
        ...
        ax2.set_title(
            "Bias Distribution by Receptor Subtype (Top 15)",
            fontsize=14,
            fontweight="bold",
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "receptor_subtype_analysis.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
