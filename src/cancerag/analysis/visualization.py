...
class VisualizationEngine:
    ...
    def __init__(
        ...
        self.docking_helper = DockingAnalyzer(
            str(self.output_dir), docking_results_path
        )
    ...
    def create_overview_dashboard(self, df: pd.DataFrame) -> None:
        ...
        # 4. LogP distribution
        fig.add_trace(go.Histogram(x=df["logp"], name="LogP", nbinsx=30), row=2, col=2)

        # 5. Receptor coverage
        receptor_counts = df["receptor"].value_counts().head(10)
        fig.add_trace(
            go.Bar(
                x=receptor_counts.index, y=receptor_counts.values, name="Top Receptors"
            ),
            row=3,
            col=1,
        )
    ...
    def create_bias_analysis_plots(self, df: pd.DataFrame) -> None:
        ...
        sns.heatmap(
            family_bias_pct, annot=True, fmt=".1f", cmap="YlOrRd", ax=axes[0, 1]
        )
        axes[0, 1].set_title(
            "Bias Distribution by Receptor Family (%)", fontweight="bold"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "bias_analysis_plots.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def create_molecular_property_plots(self, df: pd.DataFrame) -> None:
        ...
        for i, prop in enumerate(properties):
            if prop in df.columns:
                ...
                axes[i].set_title(
                    f"{prop.replace('_', ' ').title()} by Bias", fontweight="bold"
                )
                axes[i].tick_params(axis="x", rotation=45)

        plt.tight_layout()
        plt.savefig(
            self.output_dir / "molecular_property_plots.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "molecular_properties_correlation.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def create_docking_analysis_plots(self, df: pd.DataFrame) -> None:
        ...
        docking_columns = self.docking_helper.get_docking_columns(df)
        ...
        if all_affinities:
            ...
            axes[0, 0].set_title(
                "Overall Docking Affinity Distribution", fontweight="bold"
            )
        ...
        if docking_columns and docking_columns[0] in df.columns:
            ...
            if len(valid_data) > 0:
                sns.boxplot(
                    data=valid_data,
                    x="primary_bias_label",
                    y=top_receptor,
                    ax=axes[1, 0],
                )
                axes[1, 0].set_title(
                    f"Binding Affinity by Bias ({top_receptor})", fontweight="bold"
                )
        ...
        if binding_rates:
            bars = axes[1, 1].bar(
                range(len(receptor_names_binding)), binding_rates, color="lightgreen"
            )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "docking_analysis_plots.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def create_receptor_analysis_plots(self, df: pd.DataFrame) -> None:
        ...
        receptor_counts = df["receptor"].value_counts().head(10)
        bars = axes[0, 1].bar(
            range(len(receptor_counts)), receptor_counts.values, color="lightblue"
        )
        axes[0, 1].set_xlabel("Receptor")
        ...
        axes[0, 1].set_xticklabels(
            [
                ...
                for receptor in receptor_counts.index
            ],
            rotation=45,
            ha="right",
        )
        ...
        axes[1, 0].hist(
            ligand_counts, bins=20, alpha=0.7, color="lightcoral", edgecolor="black"
        )
        axes[1, 0].set_xlabel("Number of Ligands per Receptor")
        axes[1, 0].set_ylabel("Number of Receptors")
        axes[1, 0].set_title(
            "Distribution of Ligand Counts per Receptor", fontweight="bold"
        )
        ...
        sns.heatmap(
            family_bias_pct, annot=True, fmt=".1f", cmap="YlOrRd", ax=axes[1, 1]
        )
        axes[1, 1].set_title(
            "Bias Distribution by Receptor Family (%)", fontweight="bold"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "receptor_analysis_plots.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def create_statistical_analysis_plots(self, df: pd.DataFrame) -> None:
        ...
        for i, feature in enumerate(features):
            if feature in df.columns:
                ...
                axes[i].set_title(
                    f"{feature.replace('_', ' ').title()} Distribution by Bias",
                    fontweight="bold",
                )
                axes[i].tick_params(axis="x", rotation=45)
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "statistical_analysis_plots.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "key_features_correlation.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
