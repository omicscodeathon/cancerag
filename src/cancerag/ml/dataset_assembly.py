...
class DatasetAssembler:
    ...
    def __init__(self, config: Dict):
        ...
        self.biasdb_data_path = os.path.join(self.paths["raw_data"], "biasdb_data.csv")
    ...
    def load_ligand_descriptors(self) -> pd.DataFrame:
        ...
        if not os.path.exists(self.ligand_descriptors_path):
            raise FileNotFoundError(
                f"Ligand descriptors file not found: {self.ligand_descriptors_path}"
            )
    ...
    def load_docking_results(self) -> pd.DataFrame:
        ...
        Raises:
            FileNotFoundError: If docking results file doesn't exist
        """
        logger.info("Loading docking affinity results...")

        if not os.path.exists(self.docking_results_path):
            raise FileNotFoundError(
                f"Docking results file not found: {self.docking_results_path}"
            )
        ...
        logger.info(
            f"Loaded docking results for {len(df)} ligands across {len([col for col in df.columns if not col.startswith('bias_')])} receptors"
        )
    ...
    def create_bias_labels(
        self,
        ligands_df: pd.DataFrame,
        docking_df: pd.DataFrame,
        bias_metadata: pd.DataFrame,
    ) -> pd.DataFrame:
        """
        ...
        def mode_or_first(series: pd.Series, default: str = "") -> str:
        ...
        metadata_map: dict[str, dict[str, str | int]] = {}
        if not bias_metadata.empty:
            for ligand, group in bias_metadata.groupby("ligand_name"):
                meta: dict[str, str | int] = {}
                for col in metadata_cols:
                    if col in group.columns:
                        meta[col] = mode_or_first(group[col])
                if "receptor_subtype" in group.columns:
                    meta["receptor_count_hint"] = int(
                        group["receptor_subtype"].dropna().nunique()
                    )
                metadata_map[ligand] = meta

        for _, ligand_row in ligands_df.iterrows():
            ligand_name = ligand_row["ligand_name"]
            meta = metadata_map.get(ligand_name, {})

            receptor = meta.get("receptor", ligand_row.get("receptor", ""))
            receptor_subtype = meta.get(
                "receptor_subtype", ligand_row.get("receptor_subtype", "")
            )
            receptor_family = meta.get(
                "receptor_family", ligand_row.get("receptor_family", "")
            )
            bias_category = meta.get(
                "bias_category", ligand_row.get("bias_category", "")
            )
            bias_pathway = meta.get("bias_pathway", ligand_row.get("bias_pathway", ""))
            reference_ligand = meta.get(
                "reference_ligand", ligand_row.get("reference_ligand", "")
            )
            ...
            bias_data.append(
                {
                    ...
                    "receptor_count_hint": meta.get("receptor_count_hint"),
                }
            )

        bias_df = pd.DataFrame(bias_data)

        grouped = bias_df.groupby("ligand_name")
        ligand_bias_summary = grouped.agg(
            {
                "bias_label": lambda x: x.mode().iloc[0]
                if len(x.mode()) > 0
                else "unknown",
                "receptor": lambda x: mode_or_first(x, "Unknown"),
                "receptor_subtype": lambda x: mode_or_first(x, ""),
                "receptor_family": lambda x: mode_or_first(x, "Unknown"),
                "bias_category": lambda x: mode_or_first(x, ""),
                "bias_pathway": lambda x: mode_or_first(x, ""),
                "reference_ligand": lambda x: mode_or_first(x, ""),
                "receptor_count_hint": lambda x: (
                    int(x.dropna().iloc[0]) if len(x.dropna()) > 0 else None
                ),
            }
        ).reset_index()

        receptor_counts = (
            grouped["receptor_subtype"]
            .nunique()
            .reset_index()
            .rename(columns={"receptor_subtype": "receptor_count_calculated"})
        )
        ligand_bias_summary = ligand_bias_summary.merge(
            receptor_counts, on="ligand_name", how="left"
        )

        ligand_bias_summary["receptor_count"] = (
            ligand_bias_summary["receptor_count_hint"]
            .fillna(ligand_bias_summary["receptor_count_calculated"])
            .fillna(0)
            .astype(int)
        )
        ligand_bias_summary = ligand_bias_summary.drop(
            columns=["receptor_count_hint", "receptor_count_calculated"]
        )
        ligand_bias_summary = ligand_bias_summary.rename(
            columns={"bias_label": "primary_bias_label"}
        )

        logger.info(f"Created bias labels for {len(ligand_bias_summary)} ligands")
        logger.info(
            "Bias label distribution: %s",
            ligand_bias_summary["primary_bias_label"].value_counts().to_dict(),
        )

        return ligand_bias_summary

    def merge_datasets(
        self, ligands_df: pd.DataFrame, docking_df: pd.DataFrame, bias_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        unified_df = unified_df.merge(
            bias_df, on="ligand_name", how="left", suffixes=("", "_bias")
        )

        for col in metadata_columns:
            bias_col = f"{col}_bias"
            if bias_col in unified_df.columns:
                if col in unified_df.columns:
                    unified_df[col] = unified_df[col].fillna(unified_df[bias_col])
                    unified_df = unified_df.drop(columns=[bias_col])
                else:
                    unified_df = unified_df.rename(columns={bias_col: col})
        for col in metadata_columns:
            if col not in unified_df.columns:
                unified_df[col] = ""
            unified_df[col] = unified_df[col].fillna("")

        unified_df = unified_df.merge(docking_df, on="ligand_name", how="left")
    ...
    def generate_dataset_summary(
        ...
        summary = {
            ...
            "bias_label_distribution": df["primary_bias_label"]
            .value_counts()
            .to_dict(),
            ...
            * 100,
            "receptor_coverage": len(
                [
                    col
                    for col in df.columns
                    if col in feature_groups["docking_affinities"]
                ]
            ),
    ...
    def run_assembly(self) -> Tuple[pd.DataFrame, Dict]:
        ...
        if os.path.exists(self.unified_dataset_path) and os.path.exists(
            self.dataset_summary_path
        ):
            logger.info(
                f"Unified dataset already exists at {self.unified_dataset_path}. Loading existing data..."
            )
            try:
                unified_df = pd.read_csv(self.unified_dataset_path)
                with open(self.dataset_summary_path, "r") as f:
                    import json

                    summary = json.load(f)
                logger.info(
                    f"Successfully loaded existing dataset: {len(unified_df)} samples, {len(unified_df.columns)} features"
                )
                return unified_df, summary
            except Exception as e:
                logger.warning(
                    f"Could not load existing dataset: {e}. Re-running assembly..."
                )
        ...
        # Try to load docking results, but continue if missing
        try:
            docking_df = self.load_docking_results()
        except FileNotFoundError as e:
            logger.warning(
                f"Docking results not found: {e}. Continuing without docking data..."
            )
            docking_df = pd.DataFrame()
        ...
        # Create bias labels
        bias_df = self.create_bias_labels(ligands_df, docking_df, biasdb_df)
...
if __name__ == "__main__":
    ...
    print(
        f"Missing values: {summary['missing_values']} ({summary['missing_percentage']:.2f}%)"
    )
